...
const SelectInput = (props) => {
  ...
  const handleCreate = (inputValue) => {
    ...
    setTimeout(() => {        const newOption = createOption(inputValue);
      setIsLoading(false);
  ...
  return (
    <div>
      <p class="..."> {props.label} <a class="text-red-600">*</a></p>
      
      <CreatableSelect 
        defaultValue={{ value: defaults, label: defaults }}
        maxMenuHeight={200} placeholder={ props.placeholder }
        isClearable isDisabled={isLoading} isLoading={isLoading} 
        onChange={(newValue) => {setValue(newValue)
          props.form.setValue(props.name, newValue.label);
          //added
          //props.form.setFieldValue(props.name, newValue.value);
        }}
        onCreateOption={handleCreate} options={options} value={value}
